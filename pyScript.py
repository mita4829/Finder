        ...
        for i in range(startIndex,numFiles):
            ...
            if(token != "pyScript.py"):
                ...
                if(os.path.isdir(token)):
                    ...
                    print("#Error non folder found named "+token+" in "+getCurrentDirectory())
    else:
        sys.exit("Script Aborted")
...
def changeName(startIndex,parentName):
    ...
    for i in range(startIndex,numberOfFiles()):
        ...
        if((os.path.isdir(listOfFiles[i])) == False):
            ...
            lengthOfParentName = len(parentName)
            #Test 1 .raw files
            if(fileType == ".raw"):
            ...
            #Test 2 general test
            if((rowName != parentName) and (Test1 == False) and (rowName[0:lengthOfParentName] != parentName)):
                #print("Newline = "+newLine)
                newLine = checkForDuplicates(rowName,fileType,parentName)
            ...
            if((Test1 == False) and (Test2 == False) and (rowName != parentName)):
                ...
                newLine = checkForDuplicates(partialString,fileType,parentName)
                os.rename(listOfFiles[i],newLine)
                Test3 = True
                    
        else:
            print("#Error non file found in folder "+parentName+" named "+str(line)+" in "+getCurrentDirectory())
...
def checkForDuplicates(rowName,fileType,parentName):
    newLine = rowName+fileType
